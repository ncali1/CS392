...
* Date        : 6/17/2021
* Description : use threads to add two arrays.
...
void *sum_array(void *ptr) {
    ...
    int errorval;
    if ((errorval= pthread_mutex_lock(&lock))!=0){
        fprintf(stderr, "Warning: Cannot lock mutex. %s.\n",strerror(errorval));
    }
    sum+=awnser;
    if ((errorval= pthread_mutex_unlock(&lock))!=0){
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if((num_values%2)==1){
            targs[i].length=(num_values/2)+(1+i)%2;
        }else{
