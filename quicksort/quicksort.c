 ...
 * pointer arithmetic.it
 */
static int lomuto(void *array, int left, int right, size_t elem_sz,
                  int (*comp) (const void*, const void*)) {
    // TODO
    size_t l = left;
    char *switcher = (char *)array;
    char *p = (char *)array + (left * elem_sz);
    for (size_t i = left+1; i <= right; i++){
        if(comp(switcher + (i * elem_sz), p) < 0){
            p++;
            swap(switcher + (l * elem_sz), switcher + (i * elem_sz), elem_sz);
        }
    }
    swap(p, switcher +(l * elem_sz), elem_sz);
    return l;
}
